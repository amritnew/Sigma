...
//  Sigma
//
//  Created by Vinicius Mangueira on 19/08/19.
...
struct Service<T: Decodable> {
    ...
    func put(params: [String: Any], url: String, completion: @escaping (Result<T, Error>) -> Void) {
        guard let url = URL(string: url) else {return print("Does not parse String to URL")}
        ...
        do {
            ...
            let session = URLSession.shared.dataTask(with: urlRequest) { (data , _, error) in
                ...
                do {
                    ...
                    print("Error in decode class: \(T.className()) with error: \(error.localizedDescription)")
                    DispatchQueue.main.async { completion(.failure(error)) }
                }
            }
